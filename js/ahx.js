/*
	...
	-- JAMBON --
	The trick to stick to song position and sync fx to music is to reduce the buffersize
	at function AHXMasterWebKit (l.1347)
...
function AHXOutput(player) {
	return {
		...
		Init: function(Frequency, Bits) {
			console.log('Init', Frequency, Bits);
			this.Frequency = Frequency;
			this.Bits = Bits;
			this.BufferSize = Math.floor(Frequency/50);
			//this.BufferSize = Math.floor(Frequency/100);
				...
				while(samples_to_mix) {
					...
					if(false) {
						...
						for(int i = 0; i < thiscount; i++) {
							let offset = pos[v] >> 16;
							let sample1 = VolTab[Player->Voices[v].VoiceBuffer[offset]];
							let sample2 = VolTab[Player->Voices[v].VoiceBuffer[offset+1]];
							let frac1 = pos[v] & ((1 << 16) - 1);
							let frac2 = (1 << 16) - frac1;
